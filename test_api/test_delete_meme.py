...
def test_delete_meme(fixt_delete_meme, fixt_get_post_id, fixt_get_one_meme):
    post_id = fixt_get_post_id  # id мема из фикстуры (создан заранее)
    fixt_delete_meme.full_req_delete_meme(post_id)  # удаляем мем
    ...
    with allure.step("Проверяем что данного мема нет"):
        fixt_get_one_meme.full_req_get_one_meme(post_id)  # повторный GET должен вернуть 404
