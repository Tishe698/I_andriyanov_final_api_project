...
def test_change_put_meme(fixt_change_put_meme, fixt_get_post_id, fixt_payload_change):
    post_id = fixt_get_post_id  # id мема, созданного фикстурой
    payload = copy.deepcopy(fixt_payload_change)  # чистая копия исходных данных
    payload["id"] = post_id  # API требует id в теле
    fixt_change_put_meme.full_req_change_put_meme(post_id, payload)
    ...
    with allure.step("Проверяем, что изменения сохранились"):
        ...
        assert fixt_change_put_meme.json["id"] == str(post_id)
    ...
    with allure.step("Проверяем, если передан неверный формат поля"):
        ...
        fixt_change_put_meme.full_req_change_put_meme(post_id, payload)  # ожидаем 400
